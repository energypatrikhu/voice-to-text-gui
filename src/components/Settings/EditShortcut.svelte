<script lang="ts">
  ...
  let outputPrefix =
    indexToModify !== -1 ?
      $config.input.keyboardShortcuts[indexToModify].outputPrefix
    : "";
  ...
  $: title =
    actionMode !== "none" ?
      (
        actionMode in
        $translations.settings.input.keyboardShortcuts.modal.titles
      ) ?
...
>
  <Modal header="{title}">
    {#if actionMode === "remove-shortcut"}
      ...
      <span
        >{$translations.settings.input.keyboardShortcuts.modal.shortcut}:</span
      >
    ...
    {:else}
      ...
      <span
        >{$translations.settings.input.keyboardShortcuts.modal.shortcut}{(
          buttons.length > 1
        ) ?
          " (Combo)"
        : ""}:</span
      ...
      <div class="flex gap-4">
        ...
        >
          {$translations.settings.input.keyboardShortcuts.modal.buttons[
            "add-button"
          ]}
        </Button>
        ...
        <Button
          ...
          on:click="{function () {
            ...
            if (actionMode === 'add-shortcut') {
              if (buttons.length > 0) {
                $config.input.keyboardShortcuts = [
                  ...$config.input.keyboardShortcuts,
                  shortcut,
                ];
              }
