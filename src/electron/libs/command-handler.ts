...
class CommandHandler {
      ...
      syntax:
        syntax !== null
          ...
          ? __app.config.commands.prefix +
            handler.join(` | ${__app.config.commands.prefix}`)
  ...
  private async callMacro(
    ...
    outputLocation: "return" | "write",
  ) {
    const filteredMacros = __app.macros.filter(
      (macro) => macro.handler === handler,
    );
    ...
    if (filteredMacros.length === 0) {
      this.speechSynthesis.speak(
        __app.translations.speechFeedback.commandHandler.unknownMacro,
      );
      __app.console.log(
        __app.translations.textFeedback.commandHandler.unknownMacro,
      );
      return;
    }
    ...
    if (__app.config.others.mtaConsoleInputMode) {
      return await textParser(
        `{f8}${macro.prefix ? `${macro.prefix.slice(1)} ` : "say "}${
          macro.text
        }{enter}{f8}`,
      );
    }
    ...
    return await textParser(
      `${macro.prefix ? `${macro.prefix} ` : ""}${macro.text}`,
    );
  }
  ...
  private async callCommand(
    ...
    const commandArray = command
      .slice(1)
      .replace(/\s+/g, "")
      .split(__app.config.commands.splitter);
    const cmd: CmdParsed = {
    ...
    if (
      ...
      return await this.callMacro(
        cmd.args[0],
        args.length !== 0 ? args[0] : "write",
      );
    }

    const filteredCommands = this.commands.filter((_command) =>
      typeof _command.handler === "object"
        ? anyHas(_command.handler, cmd.handler)
        : _command.handler === cmd.handler,
    );
    ...
    this.speechSynthesis.speak(
      __app.translations.speechFeedback.commandHandler.unknownCommand,
    );
    __app.console.log(
      __app.translations.textFeedback.commandHandler.unknownCommand,
    );
  }
