...
export function soundWrapper() {
	try {
		...
		__app.mainWindow.webContents.send('electron', {
			event: 'playAudio',
			data: {
				src: __app.config.feedback.sounds.mode === 'custom' ? __app.config.feedback.sounds.file.filepath : resolve(join(__app.isDev ? './resources/extraResources/audio' : './resources/audio', '/activation-deactivation.mp3')),
