...
cmd.registerCommand(
	function () {
		...
		for (const [index, { name, syntax, availability, description }] of cmd.commandList.entries()) {
			...
			commands.push(`${__app.translations.textFeedback.commands.commandList.list.availability} ${availabilityMap[availability]}`);
	...
	['both', 'Parancsok', null, ['parancsok', 'commands', 'cmd', 'cmds', 'listcmds', 'listcmd'], __app.translations.textFeedback.commands.commandList.description],
);
